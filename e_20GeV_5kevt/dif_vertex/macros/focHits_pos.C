...
void focHits_pos() {
    ...
    TH2F* hXY  = new TH2F("hXY",  "Hit map in XY plane;X [cm];Y [cm]",             100, -50, 50, 100, -50, 50);
    TH2F* hYZ  = new TH2F("hYZ",  "Hit map in YZ plane;Z [cm];Y [cm]",             1000, 700, 820, 100, -50, 50);
    TH3F* hXYZ = new TH3F("hXYZ", "3D hit distribution;X [cm];Y [cm];Z [cm]",      100, -50, 50, 100, -50, 50, 1000, 700, 820);
    ...
    while (reader.Next()) {
        ...
        for (int j = 0; j < nHits; ++j) {
            hZ->Fill(z[j]);
            // Filtra apenas hits no intervalo de Z desejado para o histograma XY
            //if (z[j] >= 700 && z[j] <= 717.85) {
            if (z[j] >= 700.0 && z[j] <= 702.0) {    
                hXY->Fill(x[j], y[j]);
            }
